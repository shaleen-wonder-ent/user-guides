...
Two regional **Managed HSM clusters** — one Primary, one DR; **FIPS 140-3 Level 3 validated** — that share the same keys via **native multi-region replication** and therefore expose the **same key URI** from both regions.

Each region's workloads reach **their own local HSM** through a **Private Endpoint** in a dedicated HSM PE spoke that is peered into that region's **existing hub**. On-prem callers reach the same HSM over **ExpressRoute** via the org's **conditional DNS forwarders**.

The global public FQDN `<hsm>.managedhsm.azure.net` is fronted by an **internal, Microsoft-managed traffic routing layer** that Azure provisions automatically as part of `az keyvault region add` (DNS TTL 5 s) — there is **no customer-deployed Azure Traffic Manager profile** and **no operator endpoint toggle**.

**Public network access on the HSM is OFF** — the data plane is reachable only on the private network.
...
| **5** | **Provision the Primary HSM** with public access already **Disabled**.<br>`az keyvault create --hsm-name ... --public-network-access Disabled` — 20–30 min wait, comes up in **Pending** state.<br>Skip ahead and create the Primary PE + DNS zone group now (Step 8 for Primary) so a jump VM in the Primary app spoke can reach the HSM for activation. | This is the HSM itself.<br>Activation in Step 6 runs over the private path. | Phase 4 §4.2–4.3 |
| **6** | **Security Domain ceremony — M-of-N**, run from a jump VM in the existing Primary app spoke (reached via the hub Bastion).<br>Gather the M+ custodians, run `az keyvault security-domain download`, store the resulting `SD.json` in **3 offline locations**, custodians keep their private keys.<br>HSM is now **Active**. | The single most critical step.<br>Lose the SD file plus M custodian keys and **every key in the HSM is permanently unrecoverable**. | Phase 5 §5.1–5.5, Appendix B |
| **7** | **Add the DR region** to the same HSM (`az keyvault region add --region <dr>`).<br>Azure provisions the DR cluster, replicates the Security Domain (no second ceremony), and wires the new pool into Microsoft's internal, platform-managed routing layer under the canonical `<hsm>.managedhsm.azure.net` FQDN (DNS TTL 5 s). | This is what makes Active-Active real: same FQDN, same key URIs, RPO ≈ 0, fully automatic global routing — no customer Traffic Manager profile to deploy or operate. | Phase 6 §6.1 + Phase 3 §3.2 |
| **8** | **Create one Private Endpoint per region** into each region's `snet-pe-hsm`, register the A record into that region's Private DNS zone via the PE's DNS zone group.<br>Confirm public network access is **Disabled** on both pools (`--public-network-access Disabled`). | After this step the only path to the HSM is the private network. | Phase 7 §7.1–7.5 |
| **9** | **Assign HSM data-plane RBAC** to the Entra groups (Administrator at `/`, Crypto Officer at `/keys`).<br>**Create the per-workload keys** (`cmk-vmdisk-prod`, `cmk-sqlmi-prod`, `cmk-app-prod`).<br>Grant each workload's managed identity least-privilege role at `/keys/<name>` only. | HSM uses its own **local RBAC**, separate from Azure RBAC.<br>Per-key scoping limits blast radius. | Phase 8 §8.1–8.4, Appendix A |
| **10** | **Wire the workloads to their keys:** Disk Encryption Set → VM disks, TDE Protector → SQL Managed Instance, SDK envelope encryption → application tier.<br>Add retry / circuit-breaker for the ~6-minute replication lag.<br>Always call the HSM at the canonical `https://<hsm>.managedhsm.azure.net/...` — never a regional sub-FQDN or PE IP. | This is where encryption actually starts using your HSM keys. | Phase 9 §9.1–9.6 |
| **11** | **Enable backups, diagnostics, and alerts:**<br>• Daily + event-triggered HSM backup to immutable storage in a third region.<br>• `AuditEvent` to Log Analytics.<br>• Alerts on SD operations, role changes, key delete/purge, auth-failure bursts, replication lag. | Backups protect against accidental key deletion / ransomware on the storage tier.<br>Logs are mandatory for audit. | Phase 10 §10.1–10.5 |
| **12** | **DR drill + Go-Live:**<br>• Validate platform-managed global routing convergence — resolve the canonical FQDN from out-of-region while the drill HSM's Primary PE is intentionally made unreachable; expect DR endpoint within one TTL (~5 s).<br>• Annual SD restore into a throwaway test HSM.<br>• Tick the Go-Live checklist: public access OFF, soft-delete + purge protection ON, **no customer Traffic Manager profile present**, backups verified, alerts firing on tests, runbooks handed over. | A DR plan you don't rehearse is not a DR plan.<br>The Go-Live checklist is the final gate before production traffic. | Phase 11 §11.2–11.5, Phase 12 §12.1 |
...
### R1 — Applications must call the HSM at the canonical FQDN

Failover of the global `<hsm>.managedhsm.azure.net` FQDN is fully automatic, handled by Microsoft's internal, platform-managed routing layer (DNS TTL 5 s) — *but only for clients that resolve this exact name*.

**What breaks it:** an application or operator script that hard-codes
- a **regional sub-FQDN** (`<hsm>.<region>.managedhsm.azure.net`), or
- a **PE IP** (`10.12.1.4` / `10.22.1.4`).

Those callers bypass the platform router and keep targeting the dead region until redeployed. In-Azure workloads using the canonical FQDN are unaffected during a real outage — their region-local Private DNS continues to resolve to the surviving region's PE automatically.

**Mitigation:**
- Enforced in [Phase 9 §9.3](https://shaleen-wonder-ent.github.io/user-guides/HSM_Implementation_Steps.html#93-application-tier-envelope-encryption).
- Audited quarterly by [§10.5 check B](https://shaleen-wonder-ent.github.io/user-guides/HSM_Implementation_Steps.html#105-periodic-configuration-audits-quarterly).
- Validated in the DR drill ([§11.2](https://shaleen-wonder-ent.github.io/user-guides/HSM_Implementation_Steps.html#112-quarterly-drill-no-production-impact) step 6) by confirming the platform router converges on the DR endpoint within one TTL after a simulated Primary outage.
- Go-Live ([§12.1](https://shaleen-wonder-ent.github.io/user-guides/HSM_Implementation_Steps.html#121-pre-go-live)) explicitly blocks on the absence of any `*.trafficmanager.net` hostname in app config (a leftover from older designs).

### R2 — Private DNS zone mis-linking

The two regional `privatelink.managedhsm.azure.net` zones look identical and live in the **same** central connectivity subscription. Linking a VNet to the **wrong** zone (e.g. a DR app spoke accidentally linked to the Primary-region zone) silently mis-routes HSM traffic across regions, breaks the low-latency Active-Active design, and surfaces only during failover — when the workload tries to reach a PE IP that does not exist in its own region.

**Mitigation:**
- Enforce **strictly one zone per region**, each linked **only** to its own region's hub + app spokes + HSM PE spoke (Step 3).
- Tag zones `region=primary` / `region=dr` so cross-links are visually obvious.
- Run the quarterly automated audit in [§10.5](https://shaleen-wonder-ent.github.io/user-guides/HSM_Implementation_Steps.html#105-periodic-configuration-audits-quarterly) (`az network private-dns link vnet list`) that fails the compliance scan if any link's location doesn't match its zone's region tag.
- During testing **always validate DNS responses** from a workload in each region — `nslookup org-hsm-prod.privatelink.managedhsm.azure.net` must return `10.12.1.4` in Primary and `10.22.1.4` in DR.
...
4. **Public-DNS / hybrid caller** (anything not on the private path above) → resolves the global canonical FQDN `<hsm>.managedhsm.azure.net` through Microsoft's **internal, platform-managed routing layer** (DNS TTL 5 s, provisioned automatically by `az keyvault region add`) → the currently-healthy regional endpoint.

   **No operator action** is required during a regional outage — the router stops returning the unhealthy region's endpoint within one TTL.
